...
SCHEMA = json.loads(Path(__file__).with_name("simulation-config-v2.schema.json").read_text())
...
def parameter_instructions(parameter=None):
    ...
    for name in names:
        ...
        rules = {key: value for key, value in field.items()
                 if key in ("type", "minimum", "maximum", "exclusiveMinimum",
                            "exclusiveMaximum", "multipleOf", "enum")}
        lines.append(f"{name}: {field['description']} JSON Schema rules: {json.dumps(rules)}")
    return "\n".join(lines)
