...
# All tables are InnoDB. Child records are deleted by their ON DELETE CASCADE
# foreign keys. Clear event parent links first so self references cannot block deletion.
"${admin[@]}" <<SQL
START TRANSACTION;
...
COMMIT;
SQL
printf 'Cleared %s events and snakelab simulations. Services remain stopped.\n' "$database"
